...
def kmer_generator_pair(request, k=15, sep_token="[SEP]"):
    # Tokenize the reads
...
def predict(request: SequenceRequest):
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        line1_r1 = [r1 for r1, inf in zip(_read(request.reads_1, request.fformat), inference)
                                    if inf['label'] == 'presence']
        line1_r2 = [r2 for r2, inf in zip(_read(request.reads_2, request.fformat), inference)
                                    if inf['label'] == 'presence']
