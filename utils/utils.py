...
def get_model(model_type_or_path, num_layers, all_layers=None, from_tf=False):
    if model_type_or_path.startswith("scibert"):
        model = AutoModel.from_pretrained(cache_scibert(model_type_or_path))
    elif "t5" in model_type_or_path:
        from transformers import T5EncoderModel

        model = T5EncoderModel.from_pretrained(model_type_or_path)
    else:
        model = AutoModel.from_pretrained(model_type_or_path, from_tf=from_tf)
    model.eval()
...
def get_tokenizer(model_type_or_path, use_fast=False):
    if model_type_or_path.startswith("scibert"):
        model_type_or_path = cache_scibert(model_type)

    if LooseVersion(trans_version) >= LooseVersion("4.0.0"):
        tokenizer = AutoTokenizer.from_pretrained(model_type_or_path, use_fast=use_fast)
    else:
        assert not use_fast, "Fast tokenizer is not available for version < 4.0.0"
        tokenizer = AutoTokenizer.from_pretrained(model_type_or_path)
