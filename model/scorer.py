...
class BERTScorer:
    ...
    def __init__(
        ...
        """
        Args:
            - :param: `model_type_or_path` (str): contexual embedding model specification, default using the suggested
                      model for the target langauge; has to specify at least one of
                      `model_type_or_path` or `lang`
            ...
            - :param: `lang` (str): language of the sentences; has to specify
                      at least one of `model_type_or_path` or `lang`. `lang` needs to be
        ...
        assert lang is not None or model_type_or_path is not None, "Either lang or model_type_or_path should be specified"
        ...
        if model_type_or_path is None:
            lang = lang.lower()
            self._model_type_or_path = lang2model[lang]
        else:
            self._model_type_or_path = model_type_or_path

        self._model_from_tf = model_from_tf

        if num_layers is None:
            self._num_layers = model2layers[self.model_type_or_path]
        else:
            self._num_layers = num_layers

        # Building model and tokenizer
        self._use_fast_tokenizer = use_fast_tokenizer
        self._tokenizer = get_tokenizer(self.model_type_or_path, self._use_fast_tokenizer)
        self._model = get_model(self.model_type_or_path, self.num_layers, self.all_layers, self._model_from_tf)
        ...
        if self.baseline_path is None:
            self.baseline_path = os.path.join(
                os.path.dirname(__file__), f"rescale_baseline/{self.lang}/{self.model_type_or_path}.tsv"
            )
    ...
    @property
    def model_type_or_path(self):
        return self._model_type_or_path
    ...
    def baseline_vals(self):
        if self._baseline_vals is None:
            if os.path.isfile(self.baseline_path):
                ...
                raise ValueError(f"Baseline not Found for {self.model_type_or_path} on {self.lang} at {self.baseline_path}")
    ...
    def hash(self):
        return get_hash(
            self.model_type_or_path, self.num_layers, self.idf, self.rescale_with_baseline, self.use_custom_baseline, self.use_fast_tokenizer
        )
    ...
    def score(self, cands, refs, verbose=False, batch_size=64, return_hash=False):
        """
        Args:
            - :param: `cands` (list of str): candidate sequence 
            - :param: `refs` (list of str or list of list of str): reference sequence 
